...
const statusOptions: { value: UserStatus; labelKey: string; color: string; icon: typeof Circle }[] = [
  { value: 'online', labelKey: 'settings.online', color: 'bg-emerald-500', icon: Circle },
  { value: 'dnd', labelKey: 'settings.doNotDisturb', color: 'bg-red-500', icon: MinusCircle },
  { value: 'offline', labelKey: 'settings.invisible', color: 'bg-gray-500', icon: EyeOff },
]
...
const statusLabel = computed(() => {
  if (!wsConnected.value) return t('common.disconnected')
  const opt = statusOptions.find((o) => o.value === myStatus.value)
  return opt ? t(opt.labelKey) : t('settings.online')
})
  ...
  <div class="flex items-center gap-2 border-t border-border/50 bg-card/50 px-2 py-1.5 backdrop-blur-sm">
    <Popover>
      ...
      <PopoverContent v-if="wsConnected" side="top" align="start" class="w-56 p-1">
        <div class="px-2 py-1.5 text-xs font-semibold text-muted-foreground">{{ $t('settings.setStatus') }}</div>
        <button
